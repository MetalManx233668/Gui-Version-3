...
class QuizStarter:
  def __init__(self, parent):
    ...
    self.user_label.place(x=33,y=150)

    self.entry_box=Entry(window)
    self.entry_box.place(x=50,y=400)
...
class Quiz:
   ...
   def __init__(self, parent):
    ...
    self.quiz_frame = Frame(parent, bg = background_color, padx=70, pady=32)
    self.quiz_frame.grid()

    randomiser()

    self.question_label=Label(window, text = questions_answers[qnum][0], font =( "Tw Cen MT","18","bold"))
    self.question_label.grid(row= 0, padx=10, pady=30)  
